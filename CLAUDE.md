...
* **Every entry carries a plain-language summary of its rule**, identical in form across all three sections. Entries flagged in error carry one thing more — the measurement showing the element passed, taken from the Round snapshot — because the rule alone explains the criterion while the measurement explains this particular verdict. Two cases must not be folded into the general one. Where the player flagged a focus failure on an element they never focused, the review says exactly that: auditing without testing is itself the lesson. Where an element can never receive focus at all, the review states that the criterion does not apply, never that the player failed to test it.
